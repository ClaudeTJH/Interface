...
def dir_traitfichier(rep_travail):
    ...
    Returns:
        (data_input, retour_transat, fichiers, final) (tupple): 
        - data_input (list): Fichiers .brut.csv ou .FINAL.csv 
        - retour_transat (list): Fichiers .ODS.CSV ou .RTE.CSV
        - fichiers: (list): Fichiers dans le repertoire de travail
        - final (Bool): Si presence du fichier .FINAL.csv 
    """
        ...
        if os.path.exists(dir_traitfichier):
            ...
            for fichier in liste_fichiers:
                if not os.path.isdir(fichier):
                    ...
                    if extension in constantes.TYPES_FICHERS:
...
def ecriture_params(params_form):
    """Sauvegarde les parametres
        des formulaires de donnees

    Args:
        kwargs:
            params_form (list): Parametres.
    ...
    with open(fic_backup_env, "w") as f:
        ...
        f.writelines(constantes.CONFIG_PNT + "\n")
        f.writelines(constantes.MOTEUR_PNT + "\n")
...
def lance_application(**kwargs):
    ...
    if lanceur in constantes.LANCEURS:
        command = constantes.LANCEURS[lanceur]   
        if len(kwargs) > 1:
...
def lecture_param(rep_travail, index):
    ...
    Args:
        kwargs:
            rep_travail (str): Repertoire de travail
            index (str): Nom de l'index dans le fichier de sauvegarde
    Returns:
        param (str): Valeur demandee
    """
    ...
    if index == "adr_retour":
        ...
        liste_params = dict(zip(constantes.LISTE_INDEX,params))
...
def p_var():
    ...
    listes_var = glob(f"{constantes.DIR_VAR}/*")
...
def recup_rep_travail_autre(client, no_bt):
    ...
    traitfichier = False
    final        = False	
    rep_client   = os.path.join(constantes.DIR_VAR, client)
...
def save_adr_retour(rep, params):
    ...
    with open(constantes.FIC_ADR_EXPE, "w") as f:
